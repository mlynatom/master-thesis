...
if __name__ == "__main__":
    ...
    argparser.add_argument("--load_in_16bit", type=bool, default=False, help="Whether to load the model in 16-bit precision.")
    argparser.add_argument("--batch_size", type=int, default=16, help="Batch size to use for computing perplexity.")
    argparser.add_argument("--add_start_token", type=bool, default=True, help="Whether to add <BOS> token to the input.")
    ...
    perplexity_evaluator = Perplexity(args.model_id, args.device, args.load_in_16bit)
    ...
    with open(os.path.join(args.output_dir, f"perplexity_results_{args.model_id.split("/")[-1]}_{args.dataset_id.split("/")[-1]}.txt"), "w") as f:
        f.write(str(result))
